...
public class Stats extends ResponseWithStatus {
  ...
  public String toString() {
    return MoreObjects.toStringHelper(Stats.class).add("status", status).add("id", id)
        .add("elapsed", elapsed).add("calls", calls).add("wait", wait).add("max", max)
        .add("min", min).add("ghs 5s", ghs5s).add("ghs av", ghsAv).add("miner_count", minerCount)
        .add("frequency", frequency).add("fan_num", fanNum).add("temp_num", tempNum)
        .add("temp_max", tempMax).toString();
  }

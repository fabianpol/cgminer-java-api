...
public class Summary extends ResponseWithStatus {
  ...
  public String toString() {
    return MoreObjects.toStringHelper(Summary.class).add("status", status).add("elapsed", elapsed)
        .add("GHS 5s", ghs5s).add("GHS av", mhsav).add("Found Blocks", foundBlocks)
        .add("Found Blocks", foundBlocks).add("Getworks", getworks).add("Accepted", accepted)
        .add("Rejected", rejected).add("Hardware Errors", hwErrors).add("Utility", utility)
        .add("Discarded", discarded).add("Stale", stale).add("Get Failures", getFailures)
        .add("Local Work", localWork).add("Remote Failures", remoteFailures)
        .add("Network Blocks", networkBlocks).add("Total MH", totalMH)
